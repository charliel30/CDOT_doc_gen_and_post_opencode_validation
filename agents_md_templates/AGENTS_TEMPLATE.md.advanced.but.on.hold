...
- ✅ Correct: `{{OUTPUT_FOLDER}}temp/plan.md`
- ❌ Wrong: `/source_documents/...`
- ❌ Wrong: `/{{OUTPUT_FOLDER}}temp/plan.md`
...
2. **Per-unit worker** (one per plan entry). Each worker gets a fresh context window and owns exactly one unit. Its only job is to produce one or two files named by the unit's slug:
   ...
   **Required file format for every worker output file:**
   - **First line** must be the unit's own top-level header at heading level `##` (h2), matching the template's section naming. Examples: `## Section 3: Project Understanding`, `## Cover Letter`, `## Appendix A`. Do **not** use `#` (h1) — that level is reserved for the document title the merger writes.
   - **Second line** must be blank.
   - **Last line** of the file must also be blank (the file must end with a newline followed by an empty line). This is critical — it is what lets the merger concatenate files without gluing the next file's header onto the previous file's last paragraph. A prior run produced output where headers appeared mid-paragraph because worker files did not end in a blank line. Do not let that happen.
   - The same header + trailing-blank-line rule applies to `<slug>_traceability.md` files.

3. **Merger** (final pass). Reads only the temp files and concatenates them **in the order given by `plan.md`** into `{{OUTPUT_FILENAME}}` and `traceability.md` at the root of `{{OUTPUT_FOLDER}}`. The merger does no synthesis and no rewriting — it is a pure concatenation step driven by the plan's `order` field, not by filename sorting or numeric guessing.

   **Required merge procedure:**
   1. Write the document title as the first line of the output file, followed by a blank line. The title uses `#` (h1). This is the only h1 in the final document.
   2. For each unit in plan order, append its `<slug>_deliverable.md` contents to the output, followed by a blank line. Then do the same for `<slug>_traceability.md` into `traceability.md`.
...
2. For each unit in the plan, in order, play a worker: read only the inputs your plan told you that unit needs, write its `<slug>_deliverable.md` (and `<slug>_traceability.md` if applicable), then **forget the unit and move on**. Do not carry forward the content of the unit you just finished — the next unit should not have it in context.
3. Between units, `ls {{OUTPUT_FOLDER}}temp/` and confirm the previous unit's file exists before starting the next one. This is your progress checkpoint across compactions.
...
4. **Check for merge-glue damage.** For each unit's top-level header, confirm it appears on a line that starts with `##` — not buried mid-line after the previous unit's last paragraph. A grep like `grep -n '^## ' {{OUTPUT_FILENAME}}` should return one line per unit. If the count is short, the merger glued files together without separators — go back and re-run the merge step with the shell recipe above.
